      ...
      <div className={`${alias === master_alias ? 'bg-blue-200 shadow-md' : ''} ${background} ${visibility} grid grid-flow-col auto-cols-min font-display-sans`}>
        ...
        <div className="w-12">{score}</div>
        {
          marks.map((mark, i) => {
            ...
            return (
              <div className={`grid justify-self-stretch w-5 ${i === searchTestIndex ? markBackground : ''} ${i === searchTestIndex ? '' : markVisibility} ${i < num_chosen && i !== searchTestIndex && background === '' && alias !== master_alias ? 'bg-gray-200' : ''}`}>
